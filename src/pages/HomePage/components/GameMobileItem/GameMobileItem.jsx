...
export const GameMobileItem = ({ obj }) => {
    const iSMyLevelUnavailable = myLevel <= obj.level && obj.levelRequirement === "strict" ? true : false;

    return (<li className={`${style.gameItem}  ${(iSMyLevelUnavailable || obj.isFull ? style.unavailable : style.available)}`}>
        <div className={style.gameHeader}>
            <div className={style.gameHeaderFlexGroup}>
                <mark className={style.gameBadgeSport}>{obj.sportType === "PADEL" ? "Падель" : "Теніс"}</mark>
